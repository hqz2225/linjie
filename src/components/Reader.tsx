...
const Reader = ({ novelTitle, chapterTitle, content, chapters, currentChapterNumber, onChapterChange }: ReaderProps) => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-800'}`}>
      {/* 顶部导航 */}
      <header className={`sticky top-0 z-10 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm p-4 flex items-center justify-between w-full`}>
        <h2 className="text-2xl font-bold">{novelTitle}</h2>
        <button
      ...
      {/* 内容区域 */}
      <main className="px-4 py-8 max-w-2xl mx-auto" style={{ fontSize: `${fontSize}px` }}>
        <h2 className="text-2xl font-bold mb-6">{chapterTitle}</h2>
        <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: content }} />
      </main>

      {/* 底部控制栏 */}
      <footer className={`sticky bottom-0 z-10 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm p-4 flex items-center justify-between w-full`}>
